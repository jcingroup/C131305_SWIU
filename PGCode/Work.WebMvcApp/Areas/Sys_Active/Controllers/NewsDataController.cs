...
{
    public class NewsDataController : BaseAction<m_消息D, a_消息D, q_消息D, 消息D>
    {
        ...
        public override ActionResult ListGrid(q_消息D sh)
        {
        ...
        public override ActionResult EditMasterNewData()
        {
            ...
            ac = new a_消息D() { Connection = getSQLConnection(), logPlamInfo = plamInfo };
            md = new m_消息D() { id = ac.GetIDX() };
        ...
        public override ActionResult EditMasterDataByID(int id)
        {
            ...
            RunOneDataEnd<m_消息D> HResult = ac.GetDataMaster(id, LoginUserId);
        ...
        {
            ViewBag.Option_NewsKind = MakeCollectDataToOptions(CodeSheet.訊息分類.ToDictionary(), false);
        }
        #endregion
        ...
        [HttpPost]
        [ValidateInput(false)]
        public String ajax_MasterUpdata(m_消息D md)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        public override String ajax_MasterGridData(q_消息D queryObj)
        {
            #region 連接BusinessLogicLibary資料庫並取得資料
            ac = new a_消息D() { Connection = getSQLConnection(), logPlamInfo = plamInfo }; ;

            RunQueryPackage<m_消息D> HResult = ac.SearchMaster(queryObj, LoginUserId);
            HandleResultCheck(HResult);
            #endregion
        ...
        [HttpPost]
        [ValidateInput(false)]
        public String ajax_UploadFineSingle(int Id, String FilesKind, String hd_FileUp_EL)
        {
            ReturnAjaxFiles rAjaxResult = new ReturnAjaxFiles();
            JavaScriptSerializer js = new JavaScriptSerializer() { MaxJsonLength = 65536 }; //64K
            #region
        ...
        [HttpPost]
        [ValidateInput(false)]
        public String ajax_UploadFineDouble(int Id, String FilesKind, String hd_FileUp_EL)
        {
            ReturnAjaxFiles rAjaxResult = new ReturnAjaxFiles();
            JavaScriptSerializer js = new JavaScriptSerializer() { MaxJsonLength = 65536 }; //64K
            #region
            ...
            try
            {
                ...
                {
                    //HandImageSave(hd_FileUp_EL, Id, ImageFileUpParm.NewsDouble, FilesKind);
                    rAjaxResult.result = true;
                    rAjaxResult.success = true;
                    rAjaxResult.FileName = hd_FileUp_EL.GetFileName();
                }
