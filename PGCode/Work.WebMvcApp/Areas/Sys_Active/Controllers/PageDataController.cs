...
{
    public class PageDataController : BaseAction<m_PageData, a_PageData, q_PageData, PageData>
    {
        ...
        public override ActionResult ListGrid(q_PageData sh)
        {
        ...
        public override ActionResult EditMasterNewData()
        {
            ...
            ac = new a_PageData() { Connection = getSQLConnection(), logPlamInfo = plamInfo };
            md = new m_PageData() { id = ac.GetIDX() };
        ...
        public override ActionResult EditMasterDataByID(int id)
        {
            ...
            RunOneDataEnd<m_PageData> HResult = ac.GetDataMaster(id, LoginUserId);
        ...
        [HttpPost]
        [ValidateInput(false)]
        public String ajax_MasterUpdata(m_PageData md)
        {
        ...
        [HttpGet]
        public override String ajax_MasterGridData(q_PageData queryObj)
        {
            #region 連接BusinessLogicLibary資料庫並取得資料
            ac = new a_PageData() { Connection = getSQLConnection(), logPlamInfo = plamInfo }; ;

            RunQueryPackage<m_PageData> HResult = ac.SearchMaster(queryObj, LoginUserId);
            HandleResultCheck(HResult);
            #endregion
            ...
            foreach (m_PageData md in Modules)
            {
